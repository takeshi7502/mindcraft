...
export const actionsList = [
    ...
    },
    {
        ...
        description: 'Find, move to, and actually break/mine the nearest block of the given type. Use only for mining exactly one nearest visible block; for quantities or user requests like mine 5 diamonds, use collectBlocks or collectAndGivePlayer.',
        params: {
    ...
    },
    {
        ...
        description: 'Collect a requested number of blocks/items, then give them to a player if that player is nearby/visible. Use for requests like mine 10 diamonds for me, get a stack of iron and give it to me. Default quantity is 1; a stack is 64.',
        params: {
        ...
        perform: runOwnerLockedTask('collectAndGivePlayer', async (agent, player_name, type, num) => {
            const collected = await skills.collectBlock(agent.bot, type, num);
            if (!collected) return taskResult(false, { message: `Could not collect ${num} ${type}.` });
            ...
            if (!player || player.isValid === false || agent.bot.entity.position.distanceTo(player.position) > 32) {
                return taskResult(true, { message: `Collected ${type}, but ${player_name} is not nearby so I will keep it for now.` });
            }
            const given = await skills.giveToPlayer(agent.bot, type, player_name, num);
            return taskResult(Boolean(given), {
                message: given ? `Collected and gave ${num} ${type} to ${player_name}.` : `Collected ${type}, but could not give it to ${player_name}.`,
            });
        }, 10)
    },
    {
